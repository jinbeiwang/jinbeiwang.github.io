...
| 色相 | 值 | 管什么 |
|---|---|---|
| 靛蓝 | `#1b4f8a` | **结构与导航** —— 分类行、返回链接、章节序号、正文链接、代码块左侧竖线、代码关键字、范围框（`box note`）。一句话：你点了会「走去别处」的都属于它 |
| 祖母绿 | `#0f7b5f` | **证据与结论** —— 二级标题上方的短横、结论框（`box takeaway`）、`[doc]` / `[lit]` / `[实践]` 出处标记、来源/状态标签、图号、表格三条线与表头底色、列表符号、引用块左边线、目录里正在读的那一条与「CONTENTS」上方的短横、文末收口线、「通过」pill。一句话：它会告诉你「我们发现了什么」 |

页头那条第 2 像素的分隔线是两个色相**唯一**的交汇处：左半靛蓝、右半祖母绿。加新元素时先问它属于上面哪一句，再用对应的颜色。第三个颜色只允许有语义理由 —— 琥珀 `cav` 是风险、红 `bad` 是失败，仅此而已。

祖母绿的短横只加在**二级标题**上（SAS 篇 7 条、前端篇 14 条），三级标题不加 —— 三级标题有 44 条，都加会让长文变成大纲，而大纲的位置在左侧抽屉里。

> 短横是用 `position:absolute` 放进标题本来就有的上边距里的，不参与排版。所以「只改颜色、不动版式」是能验证的：改版前后两篇长笔记的 `scrollHeight` 完全一致（35565 / 12174）。
