...
| 色相 | 值 | 管什么 |
|---|---|---|
| 靛蓝 | `#1b4f8a` | **结构** —— 章节序号、链接、代码块左侧竖线、返回链接、分类行 |
| 祖母绿 | `#0f7b5f` | **证据** —— 二级标题上方的小短横、结论框、`[doc]` / `[lit]` / `[实践]` 出处标记、目录里正在读的那一条、表格的三条线 |

再加一组语义色 `pill ok / cav / bad`（通过 / 注意 / 不通过），只用在标签上。

祖母绿的用量刻意压住：短横只加在**二级标题**上（SAS 篇 7 条、前端篇 14 条），三级标题不加，否则长文会被点花。
